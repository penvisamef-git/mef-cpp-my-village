...
const route = (prop) => {
    ...
    async (req, res) => {
      try {
        ...
        await UserModel.updateOne(
          { _id: saveData._id }, // condition
          {
            $set: {
              email: saveData._id + "@gmail.com",
      ...
      try {
        ...
        if (id) {
          ...
          if (!user.is_super_admin) {
            // loading role
            const role = await modelGroupUser
              .findOne({
  ...
  prop.app.get(
    ...
    prop.request_user,
    async (req, res) => {
      var filter = [{ is_super_admin: false, deleted: false }];

      if (req.query.group_user_id) {
        if (req.query.group_user_id != null) {
          filter.push({ group_user_id: req.query.group_user_id });
        }
      }
      try {
